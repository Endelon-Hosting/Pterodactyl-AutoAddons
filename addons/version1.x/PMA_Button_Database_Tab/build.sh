...
curl -sSLo $PTERO/public/phpmyadmin/config.inc.php https://raw.githubusercontent.com/Endelon-Hosting/Pterodactyl-AutoAddons/"${SCRIPT_VERSION}"/addons/version1.x/PMA_Button_Database_Tab/config.inc.php
mkdir -p $PTERO/temp
curl -sSLo $PTERO/temp/PMA_Button_Database_Tab.tar.gz https://raw.githubusercontent.com/Endelon-Hosting/Pterodactyl-AutoAddons/"${SCRIPT_VERSION}"/addons/version1.x/PMA_Button_Database_Tab/PMA_Button_Database_Tab.tar.gz
...
if [[ "$ASK_CREATE_USER" =~ [Yy] ]]; then
  ...
  while [ -z "$USERNAME" ] || create_user_check; do
    echo -ne "* Username to be created: "
    read -r USERNAME
