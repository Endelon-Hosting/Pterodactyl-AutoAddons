...
mkdir -p $PTERO/temp
curl -sSLo $PTERO/temp/Files_In_Editor.tar.gz https://raw.githubusercontent.com/Endelon-Hosting/Pterodactyl-AutoAddons/"${SCRIPT_VERSION}"/addons/version1.x/Files_In_Editor/Files_In_Editor.tar.gz
tar -xzvf $PTERO/temp/Files_In_Editor.tar.gz -C $PTERO/temp
cp -rf -- $PTERO/temp/Files_In_Editor/* "$PTERO"
rm -rf $PTERO/temp
}
